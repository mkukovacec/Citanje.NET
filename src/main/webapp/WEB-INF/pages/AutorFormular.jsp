...
<html>
	<head>
		...
		<title>Uređivanje ${zapis.imeAutora} ${zapis.prezimeAutora}</title>
				...
				<div id="main-container-image">
					<section class="work">
						<form action="" method="post">
							<fieldset>
								<div class="polja">
									<h3>${zapis.imeAutora} ${zapis.prezimeAutora}</h3>
								</div>
								<input type="hidden" name="id" value="${zapis.id}" />
								<c:if test="${zapis.imaPogresku('id')}">
									<p><c:out value="${zapis.dohvatiPogresku('id')}"/></p>
								</c:if>
								<input type="hidden" name="imeAutora" value="${zapis.imeAutora}" />
								<c:if test="${zapis.imaPogresku('imeAutora')}">
									<p><c:out value="${zapis.dohvatiPogresku('imeAutora')}"/></p>
								</c:if>
								<input type="hidden" name="prezimeAutora" value="${zapis.prezimeAutora}" />
								<c:if test="${zapis.imaPogresku('prezimeAutora')}">
									<p><c:out value="${zapis.dohvatiPogresku('prezimeAutora')}"/></p>
								</c:if>

								<div class="polja">
									<textarea class="form-control biografija" rows="30" cols="60" name="biografija">${zapis.biografija}</textarea>
									<c:if test="${zapis.imaPogresku('biografija')}">
										<p class="pogreska"><c:out value="${zapis.dohvatiPogresku('biografija')}"/></p>
									</c:if>
								</div>

								<input class="btn btn-success polja" type="submit" name="metoda" value="Pohrani">
							</fieldset>
						</form>
					</section>
				</div>
