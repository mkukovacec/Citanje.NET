					...
					<section class="work">
						...
						<c:if test="${not empty sessionScope['current.user']}">
							<a class="btn btn-success polja"
								href="/Citanje.NET/djelo/${djelo.id}/dodajKomentar">Dodaj
								komentar</a>
						</c:if>
						<legend class="legend"></legend>
						<ul>
							<c:forEach var="komentar" items="${djelo.komentari}">
								<li class="polja"><object
										data="${pageContext.request.contextPath}/avatars/${komentar.korisnik.korisnickoIme}.png"
									...
									<p>
										<a
											href="/Citanje.NET/korisnik/${komentar.korisnik.korisnickoIme}">
											${komentar.korisnik.korisnickoIme}: ${komentar.datumObjave}</a>
									</p>
									<c:set var="urediID"
										value="${zaUredivanje}" /> <c:choose>
										<c:when test="${komentar.id == urediID}">
											<form
												action="/Citanje.NET/djelo/${komentar.djelo.id}/komentari/${komentar.id}/uredi"
												method="post">
										...
										<c:otherwise>
											...
											<c:if test="${trenutni != null}">
												<c:if
													test="${isAdmin == true || komentar.korisnik.korisnickoIme == trenutni}">
													<a class="btn btn-danger"
														href="/Citanje.NET/djelo/${djelo.id}/komentari/${komentar.id}/brisi">Briši</a>
													<a class="btn btn-primary"
														href="/Citanje.NET/djelo/${djelo.id}/komentari/${komentar.id}/uredi">Uredi</a>
												</c:if>
											</c:if>
										</c:otherwise>
									</c:choose></li>
