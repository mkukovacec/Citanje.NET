...
<html>
	<head>
		...
		<body>
		<c:set var="nick" value="${sessionScope['current.user']}" />
		<!-- CACHE -->
		<div class="cache"></div>
		...
		<div id="wrapper-header">
			<div id="main-header" class="object">
				...
				<div class="row" id="main_tip_search">
					...
					<c:if test="${neprocitaneOpomene > 0 }">
						<a class="btn btn-danger pull-right" href="/Citanje.NET/korisnik/${nick}/opomene">
							${neprocitaneOpomene} opomena</a>
					</c:if>
					</c:if>
				</div>
				<div id="stripes"></div>
		...
		<div id="wrapper-navbar">
			<c:if test="${not empty nick}">
			<a href="/Citanje.NET/dodajDjelo">
				<button type="button" id="new-entry-button" class="btn btn-success"
					data-toggle="button" aria-pressed="false" autocomplete="off">
					Dodaj djelo</button>
			</a>
			</c:if>
			<a href="/Citanje.NET/autor">
				<button type="button" id="new-entry-button" class="btn btn-primary"
					data-toggle="button" aria-pressed="false" autocomplete="off">
					Popis autora</button>
			<a href="/Citanje.NET/zanr">
				<button type="button" id="new-entry-button" class="btn btn-primary"
					data-toggle="button" aria-pressed="false" autocomplete="off">
					Popis žanrova</button>
			<a href="/Citanje.NET/pretraga">
				<button type="button" id="new-entry-button" class="btn btn-info"
					data-toggle="button" aria-pressed="false" autocomplete="off">
					Traži djelo</button>
			</a> <a href="/Citanje.NET/pretragaKorisnika">
				<button type="button" id="new-entry-button" class="btn btn-info"
					data-toggle="button" aria-pressed="false" autocomplete="off">
					Traži korisnika</button>
			</a>
		</div>
		...
		<div id="wrapper-container">
			<div class="container object" id="container-object">
				<div id="main-container-image">
					<section class="work">
						<c:forEach var="d" items="${djela}">
							<figure class="white">
								<a>
									<dl>
										<a href="/Citanje.NET/djelo/${d.id}">
											<dt id="book-info">
												<h4>${d.naslov}</h4>
											</dt>
										</a>
										<a href="/Citanje.NET/autor/${d.autor.id}">
											<dt id="book-info">
												<h4>${d.autor.imeAutora} ${d.autor.prezimeAutora}</h4>
											</dt>
										</a>
										<a href="/Citanje.NET/zanr/${d.zanr.id}">
											<dd id="book-info">
												<h5>${d.zanr.naziv}</h5>
											</dd>
										</a>
										<dd id="book-info">${d.godinaIzdanja}</dd>
									</dl>
								</a>
							</figure>
						</c:forEach>
