...
<html>
	<head>
		<title>Upravljanje djelima</title>
					...
					<section class="work">
						<c:if test="${not empty sessionScope['current.user']}">
							<a class="btn btn-success polja"
								href="/Citanje.NET/dodajDjelo">Dodaj djelo</a>
						</c:if>
						...
						<ul>
							<c:forEach var="d" items="${djela}">
								<li>
									<p class="polja">
										Autor djela: <a href="/Citanje.NET/autor/${d.autor.id }">${d.autor.prezimeAutora},
											${d.autor.imeAutora}: </a>
									</p>
									<p class="polja">
										Naslov djela: <a href="/Citanje.NET/djelo/${d.id}">${d.naslov}</a>
									</p>
									<p class="polja">
										<%
											if (session.getAttribute("current.user") != null && (Boolean) session.getAttribute("current.admin")) {
										%>
										<a class="btn btn-danger"
											href="/Citanje.NET/kontrolpanel/djela/brisanje/${d.id}">Briši</a>
										<%
											}
										%>
									</p>
