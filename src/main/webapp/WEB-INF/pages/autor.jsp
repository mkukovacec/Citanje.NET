...
<html>
	<head>
		...
		<title>${autor.imeAutora} ${autor.prezimeAutora}</title>
		...
		<div id="wrapper-container">
			<div class="container object" id="container-object">
				<div id="main-container-image">
					<section class="work">
						<div>
							<h2 class="polja">${autor.imeAutora} ${autor.prezimeAutora}</h2>
							<legend class="legend"></legend>
							<h3 class="polja">Biografija:</h3>
							<p class="polja">${autor.biografija}</p>
						</div>
						<c:set var="trenutni" value="${sessionScope['current.user']}" />
						<c:if test="${trenutni!=null }">
							<a class="btn btn-primary polja"
								href="/Citanje.NET/autor/${autor.id}/uredi">Uredi</a>
						</c:if>
						<legend class="legend"></legend>
						<div class="polja">
							<p>
								Zadnja izmjena: ${autor.datumObjave} <br>
								<c:if test="${ not empty autor.zadnjiUredio }">
									<object
										data="${pageContext.request.contextPath}/avatars/${autor.zadnjiUredio.korisnickoIme}.png"
									...
									<a
										href="/Citanje.NET/korisnik/${autor.zadnjiUredio.korisnickoIme}">
										${autor.zadnjiUredio.korisnickoIme}</a>
								</c:if>
							</p>
						...
						<legend class="legend"></legend>
						<h3 class="polja">Djela:</h3>
						<ul>
							<c:forEach var="d" items="${djelaAutora}">
